@file:JvmName("CMPDataSerializer")
...
fun UsercentricsCMPData.serialize(): JSObject {
    return JSObject().apply {
        put("settings", settings.serialize())
        put("services", services.map { it.serialize() }.serialize())
        put("categories", categories.map { it.serialize() }.serialize())
        put("activeVariant", activeVariant.ordinal)
        put("userLocation", userLocation.serialize())
        put("legalBasis", legalBasis.serialize())
    }
}

private fun UsercentricsSettings.serialize(): JSObject {
    return mapOf(
...
private fun CCPASettings.serialize(): JSObject {
    return mapOf(
...
private fun TCF2Settings.serialize(): JSObject {
    return mapOf(
        "firstLayerTitle" to firstLayerTitle,
        "secondLayerTitle" to secondLayerTitle,
...
private fun UsercentricsCustomization.serialize(): JSObject {
    return mapOf(
...
private fun CustomizationFont.serialize(): JSObject {
    return mapOf(
        "family" to family,
        "size" to size,
    ).toJSObject()
...
private fun CustomizationColor.serialize(): JSObject {
    return mapOf(
        "primary" to primary,
...
private fun FirstLayer.serialize(): JSObject {
    return mapOf(
        "hideButtonDeny" to hideButtonDeny,
    ).toJSObject()
}

private fun SecondLayer.serialize(): JSObject {
    return mapOf(
...
private fun UsercentricsService.serialize(): JSObject {
    return mapOf(
...
private fun UsercentricsCategory.serialize(): JSObject {
    return mapOf(
        "categorySlug" to categorySlug,
        "label" to label,
        "description" to description,
...
internal fun UsercentricsLocation.serialize(): JSObject {
    return mapOf(
...
internal fun AdditionalConsentModeData.serialize(): JSObject {
    return JSObject().apply {
        put("acString", acString)
        put("adTechProviders", adTechProviders.map { it.serialize() }.serialize())
    }
}

private fun AdTechProvider.serialize(): JSObject {
    return mapOf(
        "consent" to consent,
        "id" to id,
        "name" to name,
...
private fun LegalBasisLocalization.serialize(): JSObject {
    return mapOf(
...
private fun TranslationAriaLabels.serialize(): JSObject {
    return mapOf(
